...
public interface LendingRepository extends CrudRepository<Lending, Long> {
    ...
    @Query("select l from Lending l where (:idClient is null or l.client.id =:idClient) and (:idGame is null or l.game.id = :idGame) and (:newDate is null or (l.begin_date <= :newDate and l.end_date >= :newDate))")
    ...
    @Query("select l from Lending l where l.game.id = :idGame and (l.begin_date <= :eDate and l.end_date >= :bDate)")
    ...
    @Query("select l from Lending l where l.client.id = :idClient and (l.begin_date <= :eDate and l.end_date >= :bDate)")
